    ...
    private fun fetchResidents() {
        ...
        lifecycleScope.launch {
            try {
                ...
                residents.clear()
                residents.addAll(result.sortedBy { it.name })
                filterResidents(binding.searchView.query.toString())
                binding.progressBar.visibility = View.GONE
            } catch (e: Exception) {
                binding.progressBar.visibility = View.GONE
                Toast.makeText(this@ResidentsActivity, getString(R.string.error_general, e.localizedMessage), Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    private fun deleteResident(id: String) {
        lifecycleScope.launch {
            try {
                ...
                Toast.makeText(this@ResidentsActivity, getString(R.string.resident_deleted), Toast.LENGTH_SHORT).show()
                fetchResidents()
            } catch (e: Exception) {
                Toast.makeText(this@ResidentsActivity, getString(R.string.delete_failed, e.localizedMessage), Toast.LENGTH_SHORT).show()
            }
        }
    }
}
