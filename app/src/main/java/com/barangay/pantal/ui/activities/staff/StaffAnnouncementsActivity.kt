    ...
    private fun deleteAnnouncement(announcement: Announcement) {
        lifecycleScope.launch {
            try {
                ...
                Toast.makeText(this@StaffAnnouncementsActivity, "Announcement deleted", Toast.LENGTH_SHORT).show()
                fetchAnnouncements()
            } catch (e: Exception) {
                Toast.makeText(this@StaffAnnouncementsActivity, "Delete failed: ${e.localizedMessage}", Toast.LENGTH_SHORT).show()
            }
        }
    }
}
