...
class StaffResidentAdapter(private var residents: List<Resident>) :
    RecyclerView.Adapter<StaffResidentAdapter.ResidentViewHolder>() {
    ...
    inner class ResidentViewHolder(private val binding: ItemStaffResidentBinding) :
        RecyclerView.ViewHolder(binding.root) {

        fun bind(resident: Resident) {
            binding.residentName.text = resident.name
            binding.residentAge.text = (resident.age ?: 0).toString()
            binding.residentAddress.text = resident.address ?: "N/A"
            binding.residentContact.text = "09171234567"
