...
class ReportAdapter(
    ...
    inner class ReportViewHolder(private val binding: ItemReportBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(report: Report) {
            binding.reportTitle.text = report.type.orEmpty().ifBlank { "General Report" }
            binding.reportDescription.text = report.details.orEmpty()
            binding.reportDate.text = SimpleDateFormat("MMM dd, yyyy - hh:mm a", Locale.getDefault()).format(Date(report.timestamp ?: System.currentTimeMillis()))
            
            // Display reporter name if available
            val reporter = if (!report.reporterName.isNullOrEmpty()) "By: ${report.reporterName}" else "ID: ${report.reporterId.orEmpty().take(8)}"
            // Using reportTitle as a container for both for now, or you can add a dedicated field
            binding.reportTitle.text = "${report.type.orEmpty().ifBlank { "General" }} ($reporter)"
