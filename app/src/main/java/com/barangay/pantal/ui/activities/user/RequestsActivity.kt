...
class RequestsActivity : BaseActivity(), RequestAdapter.OnRequestInteractionListener {
    ...
    override fun onViewRequest(request: Request) {
        ...
        if (request.status.equals("Approved", ignoreCase = true) && !request.pdfUrl.isNullOrEmpty()) {
            dialogBuilder.setNeutralButton("View Generated PDF") { _, _ ->
                openPdfPreview(request)
            }
        } else if (request.status.equals("Approved", ignoreCase = true)) {
            dialogBuilder.setNeutralButton("View Generated PDF") { _, _ ->
                openGeneratedCertificatePreview(request)
            }
        }
    ...
    private fun openPdfPreview(request: Request) {
        if (!request.pdfUrl.isNullOrEmpty()) {
    ...
    private fun openGeneratedCertificatePreview(request: Request) {
        if (!request.status.equals("Approved", ignoreCase = true)) {
            Toast.makeText(this, "Certificate is not approved yet.", Toast.LENGTH_SHORT).show()
            return
        }

        val requestAdmin = RequestAdmin(
            id = request.id,
            ...
            certificateAge = request.certificateAge,
            certificateCitizenship = request.certificateCitizenship,
            barangayArea = request.barangayArea,
            specimenSignature = request.specimenSignature,
            issueDate = request.issueDate,
            punongBarangayName = request.punongBarangayName,
            status = request.status,
            timestamp = request.timestamp,
            pdfUrl = request.pdfUrl
        )

        val intent = Intent(this, PrintCertificateActivity::class.java).apply {
            putExtra("request", requestAdmin)
